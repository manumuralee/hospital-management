    ...
    window.fetch = function (url, opts) {
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                ...
                // get patients
                if (url.endsWith('/patients') && opts.method === 'GET') {
                    ...
                    if (opts.headers && opts.headers.Authorization === 'Bearer fake-jwt-token') {
                        resolve({ ok: true, text: () => Promise.resolve(JSON.stringify(patients))});
                    } else {
                        // return 401 not authorised if token is null or invalid
                        reject('Unauthorised');
                    }
                ...
                // get patient by id
                if (url.match(/\/patients\/\d+$/) && opts.method === 'GET') {
                    // check for fake auth token in header and return user if valid, this security is implemented server side in a real application
                    if (opts.headers && opts.headers.Authorization === 'Bearer fake-jwt-token') {
                        // find user by id in patients array
                        let urlParts = url.split('/');
                        let id = parseInt(urlParts[urlParts.length - 1]);
                        let matchedPatientss = patients.filter(patient => { return patient.id === id; });
                        let patient = matchedPatientss.length ? matchedPatientss[0] : null;

                        // respond 200 OK with patient
                        resolve({ ok: true, text: () => JSON.stringify(patient)});
                    } else {
                        // return 401 not authorised if token is null or invalid
                        reject('Unauthorised');
                    }
                ...
                if (url.endsWith('/patients/add') && opts.method === 'POST') {
                    ...
                    // save new patient
                    newPatients.id = patients.length ? Math.max(...patients.map(patient => patient.id)) + 1 : 1;
